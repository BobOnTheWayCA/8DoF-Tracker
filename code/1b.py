...
# Show camera preview until user confirms to select ROI
print("Press any key to stop preview and select ROI.")
while True:
    ...
    if cv2.waitKey(1) != -1:  # Any key to stop preview
        break
cv2.destroyWindow("Camera Preview")

# Read first frame and set ROI
ret, frame = cap.read()
if not ret:
    print("Error: Unable to read from the camera.")
    cap.release()
...
# Note: static (constant) template causes unexpected drifting ROI
# Best performance for a "High-def" webcam (1920*1080) is observed with a changing template
use_static_template = False  # Set to False to update template every frame

current_roi = [x, y, w, h]
static_template_frame = frame.copy()  # Store initial template
...
while True:
    ...
    if use_static_template:
        template_frame = static_template_frame  # Use the first frame as the template
    else:
        template_frame = frame  # Update template with the last frame

    # 50 is a reasonable choice on Apple M3 Pro, since large ROI/high iter will slow down performance
    current_roi = simple_tracker(current_roi, template_frame, next_frame,
                                 max_iterations=50, threshold=1e-3)

    (nx, ny, nw, nh) = current_roi
    cv2.rectangle(next_frame, (nx, ny), (nx+nw, ny+nh), (0, 255, 0), 2)
    cv2.putText(next_frame, f"Tracking", (10, 30),
